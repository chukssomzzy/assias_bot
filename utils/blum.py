...
class Blum:
    ...
    async def login(self):
        max_retries = 20 # you no go work ke
        retry_delay = 2  # seconds - cus i no get time to waste
        ...
        for attempt in range(max_retries):
            try:
                ...
                if resp.headers.get('Content-Type', '').startswith('application/json'):
                    ...
                    raise ValueError("Response is not JSON - Retrying till the Werey works")
    ...
    async def claim_diamond_retry(self):
        resp = await self.session.post("https://game-domain.blum.codes/api/v1/daily-reward?offset=-60", proxy=self.proxy)
        txt = await resp.text()
        return True if txt == 'OK' else txt
